...
export default class SpectrogramPlugin {
  ...
  constructor(params, wf) {
    ...
    this._onReady = () => {
      const drawer = (this.drawer = wf.drawer);
      ...
      this.container =
        'string' == typeof params.container
          ...
          if (!this.container) {
            throw Error('No container for WaveForm spectrogram');
          }
          if (params.colorMap) {
          ...
          this.pixelRatio = this.params.pixelRatio || wf.params.pixelRatio;
          this.fftSamples =
            this.params.fftSamples || wf.params.fftSamples || 512;
  ...
  drawSpectrogram(frequenciesData, my) {
    ...
    const length = my.waveform.backend.getDuration();
  ...
  getFrequencies(callback) {
    ...
    const buffer = (this.buffer = this.waveform.backend.buffer);
  ...
  loadLabels(
    ...
    const freqStart = 0;
    const step =
      (this.waveform.backend.ac.sampleRate / 2 - freqStart) /
      labelIndex;
